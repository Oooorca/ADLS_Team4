...
for sparsity in sparsity_levels:
    ...
    # 保存剪枝后的模型
    os.makedirs("models_lab1_t2_new", exist_ok=True)
    # 使用 state_dict() 方式保存
    torch.save(mg_random.model.state_dict(), f"models_lab1_t2_new/random_pruned_sparsity_{sparsity}.pt")
    torch.save(mg_l1.model.state_dict(), f"models_lab1_t2_new/l1_pruned_sparsity_{sparsity}.pt")
    
    try:
        os.makedirs("models_lab1_t2_try", exist_ok=True)
        mg_random.export(f"{Path.home()}/models_lab1_t2_try/random_pruned_sparsity_{sparsity}")
        mg_l1.export(f"{Path.home()}/models_lab1_t2_try/l1_pruned_sparsity_{sparsity}")
    except Exception as e:
        print(f"Warning: failed to use 'export' for sparsity level {sparsity}: {e}")
