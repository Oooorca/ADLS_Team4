...
# 定义固定点宽度范围
fixed_point_widths = [4, 8, 16, 32]  # 修改为固定宽度值
...
# 将数据映射到 GPU（如果您希望后续 Trainer 在 GPU 上训练）
...
for width in fixed_point_widths:
    # 1. 先在 CPU 上加载模型（不要立刻 .to(device)）
    model = AutoModelForSequenceClassification.from_pretrained(checkpoint)
    model.config.problem_type = "single_label_classification"

    # 2. 创建 MaseGraph（仍在 CPU 上）
    mg = MaseGraph(
        model,
        hf_input_names=["input_ids", "attention_mask", "labels"],
    )

    # 3. 执行元数据分析 Pass（CPU 上的 dummy forward）
    mg, _ = passes.init_metadata_analysis_pass(mg)
    mg, _ = passes.add_common_metadata_analysis_pass(mg)

    # 4. 分析完成后，再把模型移到 GPU 上
    mg.model.to(device)
