...
# -----------------------------
with open("new_nas_results.json", "r") as f:
...
# 读取压缩感知搜索（带后续训练）的结果
with open("compression_results_with_post_2.json", "r") as f:
    comp_results_with_post = json.load(f)

# 读取压缩感知搜索（不带后续训练）的结果
with open("compression_results_no_post_2.json", "r") as f:
...
plt.plot(range(1, len(max_acc_without_post)+1), max_acc_without_post, label="Compression-Aware NAS (without post-training)", marker='o')
...
plt.savefig("plots/lab2_t2_new_compression_nas_comparison.png")
plt.show()
